...
public:
     ...
     *
     * Dispatches to the Tipsy reorderer (Sander, Nehab & Barczak 2007). Tipsy
     * grows vertex-cache-friendly strips by fanning around a focus vertex and
     * falls back to a stack of recently-cached vertices when the fan stalls,
     * which ties or beats the older Forsyth algorithm on irregular meshes at
     ...
     * The original Forsyth path is still available via OptimizeIndicesForsyth
     * — useful for benchmarking ACMR via MeshOptimizer::ComputeACMR.
     */
    void OptimizeIndices() {
        // Lambda wrapper resolves the ambiguity between the pointer-based
        // and vector-based overloads of OptimizeTipsy/OptimizeForsyth.
        OptimizeWith([](uint32_t* idx, size_t count, uint32_t vertexCount, uint32_t cacheSize) {
            MeshOptimizer::OptimizeTipsy(idx, count, vertexCount, cacheSize);
        });
    }

    /**
     * Reorder using the legacy Forsyth (2006) algorithm. Kept for A/B
     * measurements in the MeshOptimizer unit tests and for regressions on
     * meshes where Forsyth's tighter cache-position weighting happens to
     * beat Tipsy.
     */
    void OptimizeIndicesForsyth() {
        OptimizeWith([](uint32_t* idx, size_t count, uint32_t vertexCount, uint32_t cacheSize) {
            MeshOptimizer::OptimizeForsyth(idx, count, vertexCount, cacheSize);
        });
    }

private:
    // Internal helper: apply a reorderer per submesh range (or once over
    // the whole buffer when no submeshes are declared). Both public entry
    // points funnel through here so the submesh iteration logic lives in
    // exactly one place.
